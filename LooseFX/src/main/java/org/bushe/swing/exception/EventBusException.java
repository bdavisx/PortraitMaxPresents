...
/**
 ...
 <p/>
 Swing exceptions usually occur on the Swing Event Dispatch Thread, and often occur when code puts events on the
 EDT. This code is often in a non-EDT thread such as a thread that is receiving data from a server.  If the non-EDT
 threads puts a call on the EDT and that EDT call causes and exception, the stack trace of the exception is lost,
 and it often difficult or impossible to determine where the non-EDT call came from.
 <p/>
 ...
 <p/>
 The swing calling stack is different from the cause of the exception since it is gathered before the exception
 occurs in a different stack from the cause and used after the exception in a new thread occurs.
...
public class EventBusException extends RuntimeException {
    ...
    /**
     ...
     the Swing EDT thread.  The other is the stack of the actual thread the exception occurred on, the exception
    ...
    /**
     Client exceptions often have two stacks - one thread causes the posting of an action on another thread -
     usually the Swing EDT thread.  The other is the stack of the actual thread the exception occurred on.
