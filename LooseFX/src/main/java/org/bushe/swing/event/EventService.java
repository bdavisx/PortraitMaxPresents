...
/**
 ...
 <p/>
 In class-based pub/sub, {@link EventSubscriber}s subscribe to a type on an {@link EventService}, such as the {@link
org.bushe.swing.event.EventBus}, by providing a class, interface or generic type.  The EventService notifies
 subscribers when objects are published on the EventService with a matching type.  Full class semantics are
 respected.  That is, if a subscriber subscribes to a class, the subscriber is notified if an object of that class
 is publish or if an object of a subclass of that class is published. Likewise if a subscriber subscribes to an
 interface, it will be notified if any object that implements that interface is published.  Subscribers can
 ...
 be "exact")
 <p/>
 In topic-based pub/sub, an object "payload" is published on a topic name (String).  {@link EventTopicSubscriber}s
 subscribe to either the exact name of the topic or they may subscribe using a Regular Expression that is used to
 match topic names.
 <p/>
 ...
 handle double-subscription requests by returning false on subscribe().  A single EventSubscriber can subscribe to
 more than one event class, and a single EventTopicSubscriber can subscribe to more than one topic name or pattern.
 ...
 considered two different subscriptions and the subscriber will be called twice for the publication for a single
 event of the exact type.
 <p/>
 By default the EventService only holds WeakReferences to subscribers.  If a subscriber has no references to it,
 then it can be garbage collected.  This avoids memory leaks in exchange for the risk of accidentally adding a
 listener and have it disappear unexpectedly.  If you want to subscribe a subscriber that will have no other
 reference to it, then use one of the subscribeStrongly() methods, which will prevent garbage collection.
 <p/>
 ...
 front of the list (the more negative, the more to the front).  Those subscribers that implement Prioritized and
 return a positive priority are moved to the end of the list (the more positive, the more to the back).  The FIFO
 guarantee is only valid for the same subscribe() call.  That is, the order of two subscribers, one to List.class
 and the other to ArrayList.class is not guaranteed to be in the order of subscription when an ArrayList is
 published.  The same is true for topic subscribers when using RegEx expressions - when "Foo" is published, the
 order of subscribers that are subscribed to "Foo", "Fo*" and "F*" are not guaranteed, though the second "Fo*"
 subscriber will never be called before the first "Fo*" subscriber (ditto List and ArrayList). Prioritized
 subscribers are always guaranteed to be in the order of priority, no matter the call or the resulting mix of
 subscribers.  All ordering rules apply to all types subscribers: class, topic, pattern, veto, etc.  For Swing
 users, note that FIFO is the opposite of Swing, where event listeners are called in the reverse order of when they
 were subscribed (FILO).
 <p/>
 Publication on a class or topic name can be vetoed by a {@link VetoEventListener}. All VetoEventListeners are
 checked before any EventSubscribers or EventTopicSubscribers are called. This is unlike the JavaBean's
 VetoPropertyEventListener which can leave side effects and half-propogated events. VetoEventListeners are
 subscribed in the same manner as EventSubscribers and EventTopicSubscribers.
 <p/>
 The state of a published event can be tracked if an event or a topic's payload object implements the {@link
org.bushe.swing.event.PublicationStatus} interface.  EventServices are required to set such objects' {@link
org.bushe.swing.event.PublicationStatus} at the appropriate times during publication.
 <p/>
 ...
 collected, this is not necessary if you use subscribeStrongly()");
 </pre>
 <p/>
...
public interface EventService {
    ...
    /**
     Use this method to publish generified objects to subscribers of Types, i.e. subscribers that use {@link
    #subscribe(Type, EventSubscriber)}, and to publish to subscribers of the non-generic type.
     <p/>
     Due to generic type erasure, the type must be supplied by the caller.  You can get a declared object's type by
     using the {@link org.bushe.swing.event.generics.TypeReference} class.  For Example:
     <pre>
    ...
    /**
     ...
     objects of subclasses of the class are published, when objects implementing any of the interfaces of the class
     are published, or when generic types are published with the class' raw type.
     <p/>
    ...
    /**
     Subscribe an EventSubscriber to publication of generic Types. Subscribers will only be notified for
     publications using  {@link #publish(java.lang.reflect.Type, Object)}.
     <p/>
     Due to generic type erasure, the type must be supplied by the publisher.  You can get a declared object's type
     by using the {@link org.bushe.swing.event.generics.TypeReference} class.  For Example:
     <pre>
     TypeReference&#60;List&#60;Trade&#62;&#62; subscribingTypeReference = new TypeReference&#60;List&#60;Trade&#62;&#62;(){};
     EventBus.subscribe(subscribingTypeReference.getType(), mySubscriber);
     EventBus.subscribe(List.class, thisSubscriberWillGetCalledToo);
    ...
    /**
     Subscribes an EventSubscriber to the publication of objects exactly matching a type.  Only a
    ...
    /**
     Subscribes an EventTopicSubscriber to the publication of a topic name.  Only a <b>WeakReference</b> to the
     subscriber is held by the EventService.
     <p/>
     Subscription is weak by default to avoid having to call unsubscribe(), and to avoid the memory leaks that would
     occur if unsubscribe was not called.  The service will respect the WeakReference semantics.  In other words, if
     the subscriber has not been garbage collected, then the onEvent will be called normally.  If the hard reference
    ...
    /**
     ...
     <p/>
     The semantics are the same as {@link #subscribe(Class, EventSubscriber)}, except that the EventService holds a
     regularly reference, not a WeakReference.
     <p/>
    ...
    /**
     ...
     <p/>
     The semantics are the same as {@link #subscribe(java.util.regex.Pattern, EventTopicSubscriber)}, except that
     the EventService holds a regularly reference, not a WeakReference.
     <p/>
     The subscriber will remain subscribed until {@link #unsubscribe(String, EventTopicSubscriber)}  is called.
    ...
    /**
     Subscribes a VetoTopicEventListener to a topic name.  Only a <b>WeakReference</b> to the VetoEventListener is
     held by the EventService.
    ...
    /**
     ...
     <p/>
     The VetoEventListener will remain subscribed until {@link #unsubscribeVetoListener(String, VetoTopicEventListener)}
     is called.
    ...
    /**
     ...
     <p/>
     The VetoEventListener will remain subscribed until {@link #unsubscribeVetoListener(Pattern, VetoTopicEventListener)}
     is called.
    ...
    /**
     ...
     subscribed to match the exact topic name and those subscribed by a RegEx Pattern that matches the topic name.
     */
    ...
    /**
     Union of {@link #getVetoSubscribersToTopic(String)} and {@link #getVetoSubscribersByPattern(String)} Misnamed
     method, should be called {@link #getVetoSubscribers(String)}. Will be deprecated in 1.5.
    ...
    /**
     ...
     @return the veto subscribers that subscribed by pattern that will be called when an event is published on the
     topic.
     */
    public <T> List<T> getVetoSubscribersByPattern( String topic );

    /**
     Misnamed method for backwards compatibility. Duplicate of {@link #getVetoSubscribersToTopic(String)}. Out of
     sync with {@link #getSubscribers(String)}.
    ...
    /**
     Set the number of published data objects cached for a topics matching a pattern.  By default, no data are
     cached.
     <p/>
     This overrides any setting for the DefaultCacheSizePerClassOrTopic.
     <p/>
    ...
    /**
     When using annotations, an object may be subscribed by proxy.  This unsubscribe method will unsubscribe an
     object that is subscribed with a ProxySubscriber.
     <p/>
     If an object is subscribed by proxy and it implements EventSubscriber, then the normal unsubscribe methods will
     still unsubscribe the object.
