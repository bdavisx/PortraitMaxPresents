...
/**
 ...
 <p/>
 The EventBus allows operation in two modes - using java.util.logging so that the EventBus can be deployed in its
 own jar or using any logging system supported by apache commons logging, which of course requires other jars.
 <p/>
 The EventBus logging uses the names of its classes as the log, primarily "org.bushe.swing.event.EventService".
 This aids in debugging which subscription and publication issues.
 <p/>
 Implementation note: There are no imports in this class to make things explicit.  There is also no explicit use of
 classes outside java.util, anything else is used by reflection to avoid NoClassDefFound errors on class load.
 */
public class Logger {
   ...
   org.apache.commons.logging.Log*/ Object commonsLogger;
    ...
    /**
     Returns whether this level is loggable.  If there is a misconfiguration, this will always return false.
    ...
    private Object callCommonsLogger( final String methodName, final String message, final Throwable throwable ) {
        ...
        return callCommonsLogger( METHOD_CACHE_TWO_PARAMS, methodName, CLASS_ARGS_TWO,
            new Object[]{ message, throwable } );
    }

    private Object callCommonsLogger( final Map<String, Method> cache, final String methodName,
        final Class[] classOfArgs, final Object[] args ) {
