...
public @interface VetoRuntimeTopicPatternSubscriber {
    /**
     @return name of a method (which should return a String) and whose return value will become the subscription
     topic.
     */
    ...
    /**
     @return Whether or not to subscribe to the exact class or a class hierarchy, defaults to class hierarchy
     (false).
     */
    public abstract boolean exact() default false;

    /**
     @return Whether or not to autocreate the event service if it doesn't exist on subscription, default is true. If
     the service needs to be created, it must have a default constructor.
     */
