...
/**
 ...
 returned by {@link #getSwingEventService()}.  Also by default this same instance is returned by {@link
#getEventBusService()}, is mapped to {@link #SERVICE_NAME_EVENT_BUS} and wrapped by the EventBus.
 <p/>
 ...
 <pre>
 EventServiceLocator.setEventService(EventServiceLocator.SERVICE_NAME_SWING_EVENT_SERVICE, new
 SomeSwingEventServiceImpl());
 </pre>
 Or use system properties by:
 <pre>
...
public class EventServiceLocator {
    /**
     The name "EventBus" is reserved for the service that the EventBus wraps and is returned by {@link
    #getEventBusService}.
     */
    ...
    /**
     Set this Java property to a Class that implements EventService to use an instance of that class instead of the
     instance returned by {@link #getSwingEventService}.  Must be set before {@link #getEventBusService()} is
     called.
     */
    ...
    /**
     @param serviceName the service name of the EventService, as registered by #setEventService(String,
     EventService), or {@link #SERVICE_NAME_EVENT_BUS} or {@link #SERVICE_NAME_SWING_EVENT_SERVICE} .
     ...
     */
    public static synchronized void setEventService( final String serviceName, final EventService es )
        throws EventServiceExistsException {
        if( EVENT_SERVICES.get( serviceName ) != null && es != null ) {
            throw new EventServiceExistsException( "An event service by the name " + serviceName +
                "already exists.  Perhaps multiple threads tried to create a service about the same time?" );
        } else {
    ...
    private static synchronized EventService getEventService( final String eventServiceClassPropertyName,
        final EventService defaultService ) {
        EventService result;
        ...
        if( eventServiceClassName != null ) {
            ...
            try {
                ...
                throw new RuntimeException(
                    "Could not find class specified in the property " + eventServiceClassPropertyName +
                        ".  Class=" + eventServiceClassName, e );
            }
            Object service;
            try {
                ...
                throw new RuntimeException(
                    "InstantiationException creating instance of class set from Java property" +
                        eventServiceClassPropertyName + ".  Class=" + eventServiceClassName, e );
            } catch( final IllegalAccessException e ) {
                throw new RuntimeException(
                    "IllegalAccessException creating instance of class set from Java property" +
                        eventServiceClassPropertyName + ".  Class=" + eventServiceClassName, e );
            }
            try {
                result = (EventService) service;
            } catch( final ClassCastException ex ) {
                throw new RuntimeException( "ClassCastException casting to " + EventService.class +
                    " from instance of class set from Java property" + eventServiceClassPropertyName +
                    ".  Class=" + eventServiceClassName, ex );
            }
        } else {
