...
/**
 ...
 <p/>
 There are two derivatives of this class: The EventBusAction, which publishes the ActionEvent on the global
 EventBus, and the ContainerEventServiceAction, which publishes the ActionEvent on the a local
 ...
 {@link #setTopicName(String)} for ways to customize the topic used.  Override {@link #getTopicValue(ActionEvent)}
...
#setPublishesOnTopic(boolean)} to set this behavior.  When using class-based publication, the ActionEvent is
 published by default.  Override {@link #getEventServiceEvent(ActionEvent)} to publish an object other than the
 ActionEvent.
...
public abstract class EventServiceAction extends AbstractAction {
    ...
    /**
     ...
     @return the event service to publish on, if null and getThrowsExceptionOnNullEventService() is true (default)
    ...
    #setTopicName(String)} <li>the action's getValue("event-service-topic")  {@link #EVENT_SERVICE_TOPIC_NAME}
     <li>the action's getValue("ID") (for compatibility with the SAM ActionManager's ID) <li>the action's {@link
    ...
    /**
     ...
     <p/>
     Gets the EventService from {@link #getEventService(java.awt.event.ActionEvent)}. Checks isPublishesOnTopic().
     If true, gets the topic name from {@link #getTopicName(java.awt.event.ActionEvent)} and the topic value from
     {@link #getTopicValue(ActionEvent)}, and publishes the value on the topic on the EventService.  If false, gets
     event from {@link #getEventServiceEvent(java.awt.event.ActionEvent)}, and publishes the event on the
     EventService.
     <p/>
        ...
        if( eventService == null ) {
            if( getThrowsExceptionOnNullEventService() ) {
                throw new RuntimeException(
                    "Null EventService supplied to EventServiceAction with name:" + getName() );
            } else {
                return;
            }
        }
