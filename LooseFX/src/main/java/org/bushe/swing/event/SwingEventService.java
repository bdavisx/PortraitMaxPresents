...
/**
 ...
 <p/>
 This class is Swing thread-safe.  All publish() calls NOT on the Swing EventDispatchThread thread are queued onto
 the EDT.  If the calling thread is the EDT, then this is a simple pass-through (i.e the subscribers are notified on
 the same stack frame, just like they would be had they added themselves via Swing addXXListener methods).
...
public class SwingEventService extends ThreadSafeEventService {
    ...
    /**
     ...
     milliseconds causes an EventSubscriberTimingEvent to be published.  You can add a subscriber to this event or
     set subscribeTimingEventsInternally to true to cause the default logging to occur through the protected {@link
    #subscribeTiming(SubscriberTimingEvent)} call.
     <p/>
     Note that if you use event to launch a modal dialog, the timings will be as long as the dialog is up - this is
     the way Swing works.
     ...
     @param subscribeTimingEventsInternally add a subscriber to the EventSubscriberTimingEvent internally and call
     the protected {@link #subscribeTiming(SubscriberTimingEvent)} method when they occur.  This logs a warning to
     the {@link Logger} logger by default.
    ...
    /**
     Same as ThreadSafeEventService.publish(), except if the call is coming from a thread that is not the Swing
     Event Dispatch Thread, the request is put on the EDT through a a call to SwingUtilities.invokeLater().
     Otherwise this DOES NOT post a new event on the EDT.  The subscribers are called on the same EDT event, just
     like addXXXListeners would be.
     */
    @Override
        ...
        if( SwingUtilities.isEventDispatchThread() ) {
            ...
            SwingUtilities.invokeLater( new Runnable() {
                ...
                public void run() {
                    ...
                    SwingEventService.super
                        .publish( event, topic, eventObj, subscribers, vetoSubscribers, callingStack );
                }
            } );
        }
    }
