...
/**
 ...
 in other words, it is shared only among components within a Swing Container.  The only difference between a
 Container EventService and any other EventService is that it's found and used by the children of a container.  The
 API and available implementations all work the same as any other EventService.
 <p/>
 ...
 automatically as needed.  You only want to implement this interface when you want to limit events to subscribers in
 containers smaller than a JRootPane, such as a Form's JPanel.
