...
/**
 This event is published internally to report timing for subscribers on an EventService.  Applications may subscribe
 to this event to do handle subscribers that take too long.
...
public class SubscriberTimingEvent extends AbstractEventServiceEvent {
    ...
    /**
     ...
     null
     @param vetoEventListener the vetoEventListener that took too long, can be null if the eventListener is not
     null
     */
    public SubscriberTimingEvent( final Object source, final Long start, final Long end,
        final Long timeLimitMilliseconds,
        ...
        try {
            stringified =
                "Time limit exceeded for " + type + ". Handling time=" + (end.longValue() - start.longValue()) +
